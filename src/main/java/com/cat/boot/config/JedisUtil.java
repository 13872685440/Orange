...
public class JedisUtil {
	 ...
	 * @return
	 */
	public Boolean sismember(String key, String member, int indexdb) {
		Jedis jedis = null;
		Boolean res = null;
		try {
			jedis = jedisPool.getResource();
	...
	public Long zadd(String key, double score, String member,int indexdb) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = jedisPool.getResource();
	 ...
	 * @return 指定区间内的有序集成员的列表。
	 */
	public Set<String> zrange(String key, long min, long max,int indexdb) {
		Jedis jedis = null;
		try {
			jedis = jedisPool.getResource();
			jedis.select(indexdb);
	...
	public Set<String> zrangebyscore(String key, String max, String min,int indexdb) {
		Jedis jedis = null;
		Set<String> res = null;
		try {
			jedis = jedisPool.getResource();
	...
	public Long zremrangeByScore(String key, double start, double end,int indexdb) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = jedisPool.getResource();
	...
	public String getPath(String key) {
		return hget(RedisConst.path_key, key, RedisConst.path_db);
	}
}
